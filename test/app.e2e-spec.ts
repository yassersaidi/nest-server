        ...
        describe("POST /auth/register", () => {
            ...
            it("Should return 409 Conflict with message of yasserdx@mail.com already exist", async () => {
                ...
                return request(httpServer)
                    ...
                    .then(({ body }) => {
                        expect(body).toHaveProperty("message", "yasserdx@mail.com already exists");
        ...
        describe("POST /auth/verify-email", () => {
            ...
            it("Should return 400 BAD_REQUEST with message A code has already been sent.", async () => {
                return request(httpServer)
                    ...
                    .expect(HttpStatus.BAD_REQUEST)
                    .then(({ body }) => {
                        expect(body).toHaveProperty("message", "A code has already been sent.");
        ...
        describe("POST /auth/forgot-password", () => {
            ...
            it("Should return 400 BAD_REQUEST with message A code has already been sent.", async () => {
                return request(httpServer)
                    ...
                    .then(({ body }) => {
                        expect(body).toHaveProperty("message", "A code has already been sent.");
                        expect(body).toHaveProperty("errorSource", "Password_reset Verification Service");
                    });
            })
        })
        ...
        describe("POST /auth/reset-password", () => {
            it("Should return 400 BAD_REQUEST with message multiple errors", async () => {
                return request(httpServer)
                    ...
                    .expect(HttpStatus.BAD_REQUEST)
                    .then(({ body }) => {
                        const resetPassword = authResponseErrors.resetPassword.multipleErrors
            ...
            it("Should return 400 BAD_REQUEST with code validation error", async () => {
                return request(httpServer)
                    .post("/api/v1/auth/reset-password")
                    .send({ email: loginUserDto.email, code: "454545d", password: "newpassword20" })
                    .set('Authorization', `Bearer ${accessToken}`)
                    .expect(HttpStatus.BAD_REQUEST)
                    .then(({ body }) => {
                        expect(body).toHaveProperty("message", ["code must be shorter than or equal to 6 characters"]);
            ...
            it("Should return 400 BAD_REQUEST with message User with email: testing@email.com not found.", async () => {
                return request(httpServer)
                    .post("/api/v1/auth/reset-password")
                    .send({ email: "testing@gmail.com", code: "454545", password: "newpassword20" })
                    .set('Authorization', `Bearer ${accessToken}`)
                    .expect(HttpStatus.BAD_REQUEST)
                    .then(({ body }) => {
                        expect(body).toHaveProperty("message", "User with email: testing@gmail.com not found.");
            ...
            it("Should return 400 BAD_REQUEST with message Invalid or expired code.", async () => {
                return request(httpServer)
                    .post("/api/v1/auth/reset-password")
                    .send({ email: loginUserDto.email, code: "454545", password: "newpassword20" })
                    .set('Authorization', `Bearer ${accessToken}`)
                    .expect(HttpStatus.BAD_REQUEST)
                    .then(({ body }) => {
                        expect(body).toHaveProperty("message", "Invalid or expired code.");
                        expect(body).toHaveProperty("resolution", "Please request a new Password_reset code.");
                        expect(body).toHaveProperty("errorSource", "Password_reset Verification Service");
                    });
            })
            ...
            it("Should successful reset user password and returns message Password successfully reset. with 200 OK", async () => {
                await request(httpServer)
                    .post("/api/v1/auth/reset-password")
                    .send({ email: loginUserDto.email, code: codes.password_reset, password: "newpassowrd2024" })
                    .set('Authorization', `Bearer ${accessToken}`)
                    .expect(HttpStatus.OK)
                    .then(({ body }) => {
        ...
        describe("POST /auth/rt", () => {
            it("Should return 401 Unauthorized with message Your refresh token is missing", async () => {
                return request(httpServer)
                    ...
                    .expect(HttpStatus.UNAUTHORIZED)
                    .then(({ body }) => {
                        expect(body).toHaveProperty("message", "Your refresh token is missing");
            ...
            it("Should return 401 Unauthorized with message Your access token is expired", async () => {
                return request(httpServer)
                    ...
                    .then(({ body }) => {
                        ...
                        expect(body).toHaveProperty("message", "Your refresh token is expired");
                        expect(body).toHaveProperty("url", "/api/v1/auth/rt");
                        expect(body).toHaveProperty("from", "Refresh Token Guard");
                    });
            });
            ...
            it(`Should return Session not found or already logged out when passing old refresh token cookie`, async () => {
                return request(httpServer)
                    ...
                    .then(({body}) => {
                        console.log(body)
                        expect(body).toHaveProperty("message", "Session not found or already logged out");
